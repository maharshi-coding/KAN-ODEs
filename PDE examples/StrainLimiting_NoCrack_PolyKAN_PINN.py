...
"""
Standalone no-crack full-square Chebyshev polynomial KAN-PINN for:
...
Boundary conditions:
    G1, x = 0: Phi = sigma0 * L
    G2, x = 1: Phi = 0
    G3, y = 0: Phi = -sigma0 * (x - L)
    G4, y = 1: Phi = sigma0 * (L - x)

This is the polynomial-basis counterpart to the Gaussian no-crack script. It has
no crack, no notch, no G5 boundary, no excluded void, no tip sampling, and no
tip residual weighting. The KAN layers use Chebyshev polynomial basis functions
T_0..T_p. The default polynomial order is p=5.

Quick smoke test:
    KAN_PINN_RUN_NAME=poly_smoke KAN_PINN_EPOCHS=2 KAN_PINN_FINETUNE_EPOCHS=0 \
    KAN_PINN_PRETRAIN_EPOCHS=0 KAN_PINN_PDE_RAMP_EPOCHS=1 KAN_PINN_LBFGS_EPOCHS=0 \
    KAN_PINN_NU=16 KAN_PINN_NB=8 KAN_PINN_VAL_NU=16 KAN_PINN_VAL_NB=8 \
    KAN_PINN_HIDDEN=8 KAN_PINN_POLY_ORDER=3 \
    python3 StrainLimiting_NoCrack_PolyKAN_PINN.py
"""
...
BOUNDARY_DISPLAY = {"G1": "Gamma1", "G2": "Gamma2", "G3": "Gamma3", "G4": "Gamma4"}
...
class ChebyshevKANLayer(nn.Module):
    """
    KAN layer using Chebyshev polynomial basis functions T_0..T_p.

    Inputs are mapped to [-1, 1] before evaluating the basis. For the first
    layer, x and y live in [0, 1]. Hidden activations are tanh outputs and
    already live in [-1, 1].
    """
    ...
    def __init__(
        ...
        self.coeff = nn.Parameter(scale * torch.randn(out_dim, in_dim, poly_order + 1))
        self.lin = nn.Parameter(scale * torch.randn(out_dim, in_dim))
        self.bias = nn.Parameter(torch.zeros(out_dim))

    def _scale_to_chebyshev_domain(self, x: torch.Tensor) -> torch.Tensor:
        lo, hi = self.input_range
        if math.isclose(hi, lo):
            raise ValueError("Invalid input_range for ChebyshevKANLayer.")
        return 2.0 * (x - lo) / (hi - lo) - 1.0

    def _basis(self, x: torch.Tensor) -> torch.Tensor:
        z = self._scale_to_chebyshev_domain(x)
        terms = [torch.ones_like(z), z]
        for _ in range(2, self.poly_order + 1):
            terms.append(2.0 * z * terms[-1] - terms[-2])
        return torch.stack(terms[: self.poly_order + 1], dim=-1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        basis = self._basis(x)
        basis_part = torch.einsum("nib,oib->no", basis, self.coeff)
        lin_part = x @ self.lin.t()
        return basis_part + lin_part + self.bias.view(1, -1)


class PolyKANPINN(nn.Module):
    def __init__(self, hidden: int = 96, poly_order: int = 5):
        super().__init__()
        self.k1 = ChebyshevKANLayer(2, hidden, poly_order, input_range=(0.0, 1.0))
        self.k2 = ChebyshevKANLayer(hidden, hidden, poly_order, input_range=(-1.0, 1.0))
        self.k3 = ChebyshevKANLayer(hidden, hidden, poly_order, input_range=(-1.0, 1.0))
        self.k4 = ChebyshevKANLayer(hidden, 1, poly_order, input_range=(-1.0, 1.0))
...
def validate_configuration(mat: MaterialParams, geo: GeometryParams, trn: TrainParams) -> None:
    ...
    if trn.poly_order < 1:
        raise ValueError("KAN_PINN_POLY_ORDER must be >= 1.")
...
def pde_loss(
    ...
    if chunk_size is None or chunk_size <= 0 or chunk_size >= n:
        residual = pde_residual(model, interior_xy, mat, create_graph=create_graph, grad_norm_eps=trn.grad_norm_eps)
        return torch.mean(pde_residual_objective(residual, trn))

    total = torch.zeros((), dtype=torch.float32, device=interior_xy.device)
    for s in range(0, n, chunk_size):
        e = min(s + chunk_size, n)
        xy_chunk = interior_xy[s:e]
        residual = pde_residual(model, xy_chunk, mat, create_graph=create_graph, grad_norm_eps=trn.grad_norm_eps)
        total = total + torch.mean(pde_residual_objective(residual, trn)) * (e - s)
    return total / n
...
def boundary_loss_terms(model: nn.Module, bdata_t: Dict[str, torch.Tensor], bc: BCParams) -> Dict[str, torch.Tensor]:
    ...
    for label in BOUNDARY_LABELS:
        ...
        losses[label] = torch.mean((pred - target) ** 2)
    return losses


def boundary_loss(model: nn.Module, bdata_t: Dict[str, torch.Tensor], bc: BCParams) -> torch.Tensor:
    return torch.stack(list(boundary_loss_terms(model, bdata_t, bc).values())).mean()
...
def streaming_pde_backward(
    ...
    while s < n_total:
        ...
        try:
            xy = to_tensor(interior_np[s:e], device, requires_grad=True)
            loss = pde_loss(model, xy, mat, trn, create_graph=True)
            frac = (e - s) / n_total
            (trn.lambda_pde * pde_weight * frac * loss).backward()
            weighted_mean += float(loss.detach().cpu()) * (e - s)
            del xy, loss
            s = e
            ...
            if chunk <= 1:
                raise RuntimeError("OOM at PDE chunk size 1.")
...
def streaming_pde_eval(
    ...
    while s < n_total:
        e = min(s + chunk, n_total)
        xy = to_tensor(interior_np[s:e], device, requires_grad=True)
        loss = pde_loss(model, xy, mat, trn, create_graph=False)
        total = total + loss * (e - s)
        del xy, loss
        s = e
...
def streaming_energy_backward(
    ...
    while s < n_total:
        e = min(s + chunk, n_total)
        xy = to_tensor(interior_np[s:e], device, requires_grad=True)
        loss = energy_loss(model, xy, mat, trn, create_graph=True)
        frac = (e - s) / n_total
        (trn.lambda_energy * energy_weight * frac * loss).backward()
        weighted_mean += float(loss.detach().cpu()) * (e - s)
        del xy, loss
        s = e
    return weighted_mean / n_total
...
def streaming_energy_eval(
    ...
    while s < n_total:
        e = min(s + chunk, n_total)
        xy = to_tensor(interior_np[s:e], device, requires_grad=True)
        loss = energy_loss(model, xy, mat, trn, create_graph=False)
        total = total + loss * (e - s)
        del xy, loss
        s = e
    return total / n_total
...
def adaptive_residual_points(
    ...
    candidates = sample_interior_points(geo, max(n_select, int(trn.adaptive_candidates)))
    ...
    for s in range(0, candidates.shape[0], trn.val_pde_chunk_size):
        ...
        scores.append(torch.abs(residual).detach().cpu().numpy())
        del xy, residual
...
def residual_statistics(model: nn.Module, mat: MaterialParams, geo: GeometryParams, trn: TrainParams, device: torch.device):
    ...
    residual = pde_residual(model, xy, mat, create_graph=False, grad_norm_eps=trn.grad_norm_eps)
    values = residual.detach().cpu().numpy()
...
def grid_finite_check(model: nn.Module, geo: GeometryParams, device: torch.device, nx: int = 121, ny: int = 121):
    ...
    return {"total": int(phi.size), "bad": int(np.sum(~np.isfinite(phi)))}
...
def field_diagnostics_on_grid(
    ...
    xs = np.linspace(geo.xmin, geo.xmax, trn.pointwise_nx, dtype=np.float32)
    ys = np.linspace(geo.ymin, geo.ymax, trn.pointwise_ny, dtype=np.float32)
    xx, yy = np.meshgrid(xs, ys)
    grid = np.stack([xx.ravel(), yy.ravel()], axis=1).astype(np.float32)
    ...
    for s in range(0, grid.shape[0], trn.pointwise_batch_size):
        e = min(s + trn.pointwise_batch_size, grid.shape[0])
        ...
        residual_batch = pde_residual(model, xy, mat, create_graph=False, grad_norm_eps=trn.grad_norm_eps)
        phi[s:e] = phi_batch.detach().cpu().numpy().astype(np.float32)
        tau_np = tau_batch.detach().cpu().numpy().astype(np.float32)
        grad_mag[s:e] = tau_np
        tau_eq[s:e] = tau_np
        residual[s:e] = residual_batch.detach().cpu().numpy().astype(np.float32)
        del xy, phi_batch, tau_batch, residual_batch

    return {
        "xs": xs,
        "ys": ys,
        "phi": phi.reshape(trn.pointwise_ny, trn.pointwise_nx),
        "grad_mag": grad_mag.reshape(trn.pointwise_ny, trn.pointwise_nx),
        "tau_eq": tau_eq.reshape(trn.pointwise_ny, trn.pointwise_nx),
        "residual": residual.reshape(trn.pointwise_ny, trn.pointwise_nx),
    }
...
def save_loss_history_text(
    ...
    with out_path.open("w", encoding="utf-8") as fh:
        ...
        for idx in range(n):
            fh.write(
                f"{idx + 1},{get(loss_hist, idx):.10e},{get(pde_hist, idx):.10e},"
                f"{get(energy_hist, idx):.10e},{get(bc_hist, idx):.10e},{get(val_hist, idx):.10e}\n"
            )
    return out_path
...
def save_pointwise_loss_text(
    ...
    xx, yy = np.meshgrid(xs, ys)
    interior = np.stack([xx.ravel(), yy.ravel()], axis=1).astype(np.float32)
    boundary = sample_boundary_points(geo, trn.pointwise_boundary_each)
    ...
    with out_path.open("w", encoding="utf-8") as fh:
        ...
        for s in range(0, interior.shape[0], trn.pointwise_batch_size):
            e = min(s + trn.pointwise_batch_size, interior.shape[0])
            pts = interior[s:e]
            q = pointwise_quantities(model, pts, mat, trn, device)
            total = trn.lambda_pde * q["pde_loss"] + trn.lambda_energy * q["energy_density"]
            for i, (x, y) in enumerate(pts):
                fh.write(
                    f"interior,,{float(x):.10e},{float(y):.10e},{q['phi'][i]:.10e},nan,nan,"
        ...
        for label in BOUNDARY_LABELS:
            ...
            for s in range(0, pts.shape[0], trn.pointwise_batch_size):
                ...
                err = phi - target
                total = trn.lambda_bc * np.square(err)
                for i, (x, y) in enumerate(batch):
                    fh.write(
                        f"boundary,{label},{float(x):.10e},{float(y):.10e},"
                        f"{phi[i]:.10e},{target[i]:.10e},{err[i]:.10e},"
...
def run_cross_verification(
    ...
    residual = residual_statistics(model, mat, geo, trn, device)
    finite = grid_finite_check(model, geo, device)
    boundary = boundary_diagnostics(model, bdata_t, bc)
    print("Cross verification summary:")
    print(
        "  PDE residual | "
        f"mean|r|={residual['mean_abs']:.5e}, rms={residual['rms']:.5e}, max|r|={residual['max_abs']:.5e}"
    )
    ...
    return {"residual": residual, "finite": finite, "boundary": boundary}
...
def save_run_diagnostics(
    ...
    summary = {
        "problem": "no-crack full-square strain-limiting Chebyshev polynomial KAN-PINN",
        "basis_type": "Chebyshev polynomial",
        "poly_order": trn.poly_order,
...
def save_plots(
    ...
    fields = field_diagnostics_on_grid(model, mat, geo, trn, device)
    xs = fields["xs"]
    ys = fields["ys"]
...
def train_model(
    ...
    def checkpoint_payload(saved_epoch: int, reason: str) -> Dict[str, object]:
        ...
        return {
            ...
            "basis_type": "chebyshev_polynomial",
            "poly_order": trn.poly_order,
            "hard_bc_mode": trn.hard_bc_mode,
    ...
    if resume:
        if last_ckpt_path.is_file():
            resume_source = last_ckpt_path.name
        elif best_ckpt_path.is_file():
            resume_source = best_ckpt_path.name
    if resume_source:
        ...
        print(f"[resume] source={resume_source} epoch={completed_epochs}/{total_epochs} best_epoch={best_epoch} best_val={best_val:.6e}")
    elif resume:
    ...
    def run_stage(stage_name: str, start_epoch: int, end_epoch: int, optimizer, scheduler) -> bool:
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            if trn.max_grad_norm > 0.0:
                ...
                grad_norm = math.sqrt(sum(maybe_float(torch.sum(p.grad.detach() ** 2)) for p in model.parameters() if p.grad is not None))
            ...
            if should_log:
                ...
                print(f"  BC(train): {format_bc_loss_line({k: maybe_float(v) for k, v in bc_terms.items()})}")
            ...
            if detailed:
                ...
                print(f"  Diag(BC,val): {format_bc_loss_line({label: float(bdiag[label]['loss']) for label in BOUNDARY_LABELS})}")
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            if should_log:
                ...
                print(
                    ...
                    f"new_best={'yes' if new_best else 'no'} | ckpt={ckpt_flag} | elapsed={elapsed/60:.1f}m | ETA={eta_s/60:.1f}m"
                )
    ...
    if adam_start <= adam_end:
        ...
        stopped_early = run_stage("adam", adam_start, adam_end, opt, sch)
    else:
    ...
    if (not stopped_early) and finetune_start <= finetune_end:
        ...
        stopped_early = run_stage("finetune", finetune_start, finetune_end, opt, sch)
...
def main() -> None:
    ...
    trn = TrainParams(
        ...
        poly_order=env_int("KAN_PINN_POLY_ORDER", env_int("KAN_PINN_ORDER", 5)),
    )
    ...
    print("Starting no-crack full-square training with Chebyshev polynomial KAN.")
    print(f"Device: {device}")
    print(f"Domain: [{geo.xmin:g},{geo.xmax:g}] x [{geo.ymin:g},{geo.ymax:g}]")
    print("Internal boundaries: none")
    print(f"Polynomial basis: Chebyshev order {trn.poly_order}")
    ...
    model = PolyKANPINN(hidden=trn.hidden, poly_order=trn.poly_order).to(device)
    model.configure_boundary_ansatz(geo, bc, trn)

    root_outdir = Path(__file__).resolve().parent / "results_strainlimiting_no_crack_polynomial_python"
    ...
    boundary_points_txt = save_boundary_points_text(outdir, boundary_points_to_save, model, bc, device, boundary_points_source)
